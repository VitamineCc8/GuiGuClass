...
import com.wei.ggkt.client.course.CourseFeignClient;
import com.wei.ggkt.client.user.UserInfoFeignClient;
import com.wei.ggkt.exception.GgktException;
import com.wei.ggkt.live.mtcloud.CommonResult;
import com.wei.ggkt.live.mtcloud.MTCloud;
import com.wei.ggkt.live.service.*;
import com.wei.ggkt.model.live.*;
import com.wei.ggkt.model.user.UserInfo;
import com.wei.ggkt.model.vod.Teacher;
import com.wei.ggkt.utils.DateUtil;
import com.wei.ggkt.vo.live.LiveCourseConfigVo;
import com.wei.ggkt.vo.live.LiveCourseFormVo;
import com.wei.ggkt.vo.live.LiveCourseGoodsView;
 ...
 */
@Service
public class LiveCourseServiceImpl extends ServiceImpl<com.wei.ggkt.live.mapper.LiveCourseMapper, LiveCourse> implements LiveCourseService {
...
//         options 其他参数
        try {
            ...
            if (Integer.parseInt(commonResult.getCode()) == com.wei.ggkt.live.mtcloud.MTCloud.CODE_SUCCESS) { //成功
                //添加直播基本信息
    ...
    public void updateLiveById(LiveCourseFormVo liveCourseFormVo) {
        ...
        try {
            String res = mtCloudClient.courseUpdate(liveCourse.getCourseId().toString(),
                    ...
                    options);
            //返回结果转换，判断是否成功
            CommonResult<JSONObject> commonResult = JSON.parseObject(res, com.wei.ggkt.live.mtcloud.CommonResult.class);
            if (Integer.parseInt(commonResult.getCode()) == com.wei.ggkt.live.mtcloud.MTCloud.CODE_SUCCESS) {
                JSONObject object = commonResult.getData();
                //更新直播课程基本信息
    ...
    public JSONObject getAccessToken(Long id, Long userId) {
        ...
        try {
            String res = mtCloudClient.courseAccess(liveCourse.getCourseId().toString(),
                    ...
                    com.wei.ggkt.live.mtcloud.MTCloud.ROLE_USER,
                    3600,
                    options);
            CommonResult<JSONObject> commonResult = JSON.parseObject(res, CommonResult.class);
